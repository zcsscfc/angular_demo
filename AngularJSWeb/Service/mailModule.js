...
angular.module("mailApp", ["ngRoute"]).config(["$routeProvider", function ($routeProvider) {
    $routeProvider.when("/maillist", { controller: mailListController, templateUrl: "maillist.html" })
    .when("/view/:id", { controller: mailDetailController, templateUrl: "maildetail.html" })
    .otherwise({ redirectTo: "/maillist" });
}]);
...
function mailListController($scope) {
    $scope.mails = mails;
}

function mailDetailController($scope, $routeParams) {
